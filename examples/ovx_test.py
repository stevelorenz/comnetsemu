...
OVXCTL_DIR = path.join(
    path.expanduser("~"),
    "comnetsemu_dependencies/ovx-0.0-MAINT/OpenVirteX/utils")
...
def test_ovx():
    try:
        ...
        net = Containernet(autoStaticArp=True, autoSetMacs=True,
        ...
        info("*** Start network... \n")
        net.start()
        print("Hosts configured with IPs, switches pointing to OpenVirteX at %s:%s" % (
            ip, port))
