...
def testTopo():
    ...
    info('*** Adding hosts\n')
    center = net.addDockerHost('center', dimage='sec_test', ip='10.0.0.1', cpuset_cpus="1", cpu_quota=25000)
    client1 = net.addDockerHost('client1', dimage='sec_test', ip='10.0.0.2', cpuset_cpus="1", cpu_quota=25000)
    client3 = net.addDockerHost('client3', dimage='sec_test', ip='10.0.0.3', cpuset_cpus="0", cpu_quota=25000)
    client4 = net.addDockerHost('client4', dimage='sec_test', ip='10.0.0.4', cpuset_cpus="0", cpu_quota=25000)

    info('*** Adding switch\n')
    s1 = net.addSwitch('s1')

    info('*** Creating links\n')
    net.addLinkNamedIfce(s1, center, bw=10, delay='1ms', use_htb=True)
    net.addLinkNamedIfce(s1, client1, bw=10, delay='1ms', use_htb=True)
    net.addLinkNamedIfce(s1, client3, bw=10, delay='1ms', use_htb=True)
    net.addLinkNamedIfce(s1, client4, bw=10, delay='1ms', use_htb=True)
...
def generate_key_pair_for_host(center):
    ...
    center_privatekey = center.cmd("cat ./privatekey").replace('\n', ' ').replace('\r', '')
