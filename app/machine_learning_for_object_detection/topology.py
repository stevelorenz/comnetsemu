...
def add_ovs_flows(net):
    check_output(
        split("ovs-ofctl del-flows s1")
    )
    check_output(
        split(
            "ovs-ofctl add-flow s1 \"{proto},in_port={in_port},actions=output={out_port}\"".format(
                **{"in_port": get_ofport("s1-client"), "out_port": get_ofport("s1-vnf"), "proto": "udp"}
            )
        )
    )
    check_output(
        split(
            "ovs-ofctl add-flow s1 \"{proto},in_port={in_port},actions=output={out_port}\"".format(
                **{"in_port": get_ofport("s1-server"), "out_port": get_ofport("s1-client"), "proto": "udp"}
            )
        )
    )
...
def testTopo():
    ...
    client = net.addDockerHost(
        'client', dimage='yolov2', ip='10.0.0.11/24', cpuset_cpus="0",
        volumes=["{}".format(os.path.join(
            HOST_SRC_DIR, "preprocessor.py")) + ":" + "/app/yolov2/preprocessor.py"]
    )

    vnf = net.addDockerHost(
        'vnf', dimage='yolov2', ip='10.0.0.12/24', cpuset_cpus="0",
        volumes=["{}".format(os.path.join(
            HOST_SRC_DIR, "vnf.py")) + ":" + "/app/yolov2/vnf.py"]
    )

    server = net.addDockerHost(
        'server', dimage='yolov2', ip='10.0.0.21/24', cpuset_cpus="0",
        volumes=["{}".format(os.path.join(
            HOST_SRC_DIR, "server.py")) + ":" + "/app/yolov2/server.py"]
    )
